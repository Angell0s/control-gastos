...
import type { Category as SelectorCategory } from "@/hooks/useCategorySelector";
import {
  TagIcon,
  Cog6ToothIcon,
  BanknotesIcon,
...
export default function CategoriasPage() {
  ...
  const [allCategories, setAllCategories] = useState<Category[]>([]); // para el search (incluye activas e inactivas)
  ...
  const fetchFilteredCategories = useCallback(
    async (mode: FilterMode) => {
      ...
      setLoading(true);
      try {
        // activa/todas -> active, eliminadas -> inactive
        const apiStatus = mode === "inactive" ? "inactive" : "active";
  ...
  const fetchAllForSearch = useCallback(async () => {
    if (!token) return;
    try {
      // “all” para incluir inactivas en el selector
      const res = await api.get<Category[]>("/categories/", {
        params: { status: "all" },
      });
      setAllCategories(res.data.sort((a, b) => a.name.localeCompare(b.name)));
    } catch (err) {
      handleApiError(err, "Error al cargar catálogo de categorías");
    }
  }, [token, toast]);

  const refreshLists = useCallback(async () => {
    await Promise.all([fetchFilteredCategories(filterMode), fetchAllForSearch()]);
  }, [fetchAllForSearch, fetchFilteredCategories, filterMode]);

  // --- CARGA DE CATEGORÍAS ---
  useEffect(() => {
    fetchFilteredCategories(filterMode);
  }, [fetchFilteredCategories, filterMode]);
  ...
  const handleCategoryClick = (category: Category) => {
    if (!category.is_active) {
      toast.warning({
        ...
        description:
          "Esta categoría está en la papelera. Reactívala usando el buscador o desde el administrador.",
      });
      return;
    }
  ...
  // --- Integración Reactivación real (para el buscador) ---
  const reactivateCategory = useCallback(
    async (cat: SelectorCategory) => {
      await api.put(`/categories/${cat.id}`, { name: cat.name, is_active: true });

      // refresca listas para que:
      // - desaparezca de "Eliminadas"
      // - aparezca como activa en "Todas/Activas"
      await refreshLists();

      toast.success({
        title: "Categoría reactivada",
        description: `"${cat.name}" fue reactivada.`,
      });
    },
    [refreshLists, toast]
  );

  // Al seleccionar en el search: abrir el mismo modal de detalle
  const handleSearchSelect = useCallback(
    (categoryId: string | null) => {
      ...
      // Nota: si era inactiva, el CategorySelector ya mostró modal y reactivó antes de llegar aquí
      openCategoryDetails(cat);
    },
    [allCategories]
  );
  ...
  return (
    <div className="p-6 space-y-8 max-w-7xl mx-auto animate-in fade-in duration-500">
      ...
      {/* ✅ SEARCH arriba de las cards */}
      <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-xl p-4">
        <div className="flex items-center gap-3">
          <div className="p-2.5 rounded-xl bg-slate-100 dark:bg-slate-800 text-primary">
          ...
          <div className="flex-1">
            ...
            <p className="text-xs text-muted-foreground mb-2">
              Selecciona para ver detalles; crea o reactiva desde aquí.
            </p>
            ...
            <CategorySelector
              ...
              categories={allCategories as unknown as SelectorCategory[]}
              setCategories={setAllCategories as any}
              fetchUrl="/categories/?status=all"
              allowReactivatePrompt={true}
              placeholder="Buscar categoría..."
              onReactivateConfirm={reactivateCategory}
              onAfterCreate={async (newCat) => {
                // refresca para que la nueva aparezca en el grid según filtros
                await refreshLists();

                // abre el modal con un objeto “completo” (contadores en 0)
                openCategoryDetails({
                  id: newCat.id,
                  name: newCat.name,
                ...
                setSearchCategoryId(newCat.id);
              }}
              onAfterReactivate={async () => {
                await refreshLists();
              }}
            />
          </div>
              ...
              <div
                className={clsx(
                  ...
                  cat.is_active
                    ...
                    : "text-red-50 dark:text-red-900/10"
                )}
              >
          ...
          <div className="min-h-[200px]">
            {loadingItems ? (
              ...
              <div className="text-center py-12 flex flex-col items-center text-slate-400">
                ...
                <p>
                  No hay {viewMode === "expenses" ? "gastos" : "ingresos"} registrados
                  en esta categoría.
                </p>
              </div>
            ) : (
